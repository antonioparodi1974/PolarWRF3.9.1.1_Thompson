...
do
	...
	SUB_DIR=`dirname $i`
	PWRF_FILE=$SUB_DIR/$i
	PWRF_FILE_RENAMED=$SUB_DIR/`basename $i .PWRF3.9.1`
	WRF_FILE=`echo $PWRF_FILE_RENAMED | sed 's/\/PWRF3.9.1/\/./g'`

	if [ -f ${WRF_FILE} ] 
	then
		mv ${WRF_FILE} ${WRF_FILE}-unpolar
		ln -s $THIS_DIR/${PWRF_FILE_RENAMED}.PWRF3.9.1 $THIS_DIR/${WRF_FILE}
